...
const EditorUI = (() => {
  ...
  elements.codeButton = null;    // For inline code
  elements.codeBlockButton = null;
  elements.ulButton = null;      // Unordered list
  elements.olButton = null;      // Ordered list
  let eventCallbacks = {};
...
elements.formattingToolbar = Utils.createElement("div", { // Step 1: Create the toolbar
    id: Config.EDITOR.FORMATTING_TOOLBAR_ID || "editor-formatting-toolbar",
    className: "py-1 px-2 flex items-center space-x-1",
    classList: [Config.CSS_CLASSES.HIDDEN] // Start with the hidden class
}); // << Closing brace for the attributes object, and closing parenthesis for createElement

// Step 2: Define button details (this was moved outside the createElement call)
const formatButtonDetails = [
    ...
    },
    {
        ...
        iconHTML: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="1em" height="1em"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V5h14v14zM7 7h2v2H7zm4 0h2v2h-2zm4 0h2v2h-2z"/></svg>', // A more "blocky" icon
        title: 'Code Block',
        callbackName: 'onFormatCodeBlock'
    },
    {
        name: 'ulButton', // Unordered List
...
// Step 3: Create and append buttons to the toolbar (also moved outside)
formatButtonDetails.forEach(detail => {
  ...
  function destroyLayout() {
    ...
    // >>> START: NEW CODE for Cleanup <<<
    // Nullify references to the new toolbar and buttons
    // This helps with garbage collection and state resetting.
    const newElementsToClear = [
        'formattingToolbar', 'boldButton', 'italicButton', 'linkButton',
        'quoteButton', 'codeButton', 'codeBlockButton', 'ulButton', 'olButton'
    ];
    newElementsToClear.forEach(elName => {
        if (elements[elName]) {
            // If buttons have specific event listeners attached directly (not via Utils.createElement)
            // and not cleared by parent removal, you might need to remove them explicitly here.
            // However, Utils.createElement handles this for "eventListeners" property.
            // And removing the parent (editorContainer) should typically unbind listeners.
        }
...
const EditorManager = (() => {
  ...
  function _applyTextManipulation(type) {
    ...
    const isMarkdown = currentFileMode === Config.EDITOR.MODES.MARKDOWN;
    const isHTML = currentFileMode === Config.EDITOR.MODES.HTML; // For HTML specific tags

    let prefix = "", suffix = "", placeholder = "";
    let modifiedSegment = ""; // The part that replaces selectedTextVal or is inserted

    // Helper for block elements (multi-line selections)
    const applyToLines = (text, linePrefixTransform, isList = false, listType = 'ul') => {
        return text.split('\n').map((line, index) => {
            let currentPrefix = typeof linePrefixTransform === 'function' ? linePrefixTransform(index) : linePrefixTransform;
    ...
    switch (type) {
        ...
        case 'link':
            ...
            if (url === null) return;
            let linkDisplayText = selectedTextVal;
            if (!linkDisplayText) {
                const promptedText = prompt("Enter link text (optional):", "link text");
                if (promptedText === null) return;
                linkDisplayText = promptedText;
            }

            if (isMarkdown) {
                modifiedSegment = `[${linkDisplayText}](${url})`;
                // Position cursor to select linkDisplayText if it was newly inserted
                finalSelectionStart = textBeforeSelection.length + 1; // After '['
                finalSelectionEnd = finalSelectionStart + linkDisplayText.length;
            } else if (isHTML) {
                modifiedSegment = `<a href="${url}">${linkDisplayText}</a>`;
                finalSelectionStart = textBeforeSelection.length + `<a href="${url}">`.length;
                finalSelectionEnd = finalSelectionStart + linkDisplayText.length;
            } else { return; } // Mode not supported
            break;

        case 'quote':
            placeholder = "Quoted text";
            if (isMarkdown) {
                modifiedSegment = applyToLines(selectedTextVal || placeholder, "> ");
            } else if (isHTML) {
                modifiedSegment = `<blockquote>\n  ${selectedTextVal || placeholder}\n</blockquote>`;
            } else { return; }
            finalSelectionStart = selection.start; // Select the whole new block
    ...
    _handleEditorInput(); // This updates dirty status, status bar, preview, etc.
    EditorUI.setEditorFocus();
  }
  ...
  function enter(filePath, content) {
    ...
    EditorUI.buildLayout(DOM.terminalDiv, {
      ...
      onFormatCode: () => _applyTextManipulation('code'),         // Inline code
      onFormatCodeBlock: () => _applyTextManipulation('codeblock'), // Block of code
      onFormatUl: () => _applyTextManipulation('ul'),             // Unordered list
      onFormatOl: () => _applyTextManipulation('ol')              // Ordered list
    });
  ...
  function _updateFormattingToolbarVisibility() {
    // Ensure EditorUI and its elements are available, and the editor is active
    if (!isActiveState || !EditorUI || !EditorUI.elements || !EditorUI.elements.formattingToolbar) {
        console.warn("EditorManager: Cannot update formatting toolbar, UI elements not ready or editor not active.");
        return;
    }

    const isMarkdown = currentFileMode === Config.EDITOR.MODES.MARKDOWN;
    const isHTML = currentFileMode === Config.EDITOR.MODES.HTML; // If you want it for HTML too

    if (isMarkdown || isHTML) {
        EditorUI.elements.formattingToolbar.classList.remove(Config.CSS_CLASSES.HIDDEN);
    } else {
        EditorUI.elements.formattingToolbar.classList.add(Config.CSS_CLASSES.HIDDEN);
    }
}
  return {
...
`,
    ...
    handler: async (args, options) => {
      ...
      if (!validationResult.isValid) {
        return { success: false, error: `printscreen: ${validationResult.errorDetail}` };
      }
      const filePathArg = args[0];
      const currentUser = UserManager.getCurrentUser().name; //
      const nowISO = new Date().toISOString();
      ...
      // Using innerText to get a plain text representation of the output.
      ...
      if (existingNode) {
        ...
        // It's an existing file, check write permission
        ...
        // mtime will be updated by _updateNodeAndParentMtime

      } else {
        // --- Target Path Does Not Exist (Create New File) ---
        ...
        if (!parentNodeForCreation) {
             // This should ideally not be reached if createParentDirectoriesIfNeeded is successful and returns no error.
        ...
        // Final check on parent's write permission (createParentDirectoriesIfNeeded handles permissions for *newly created* parents,
        // but the immediate parent might have already existed).
        ...
        parentNodeForCreation.children[fileName] = {
            ...
            owner: currentUser,
            mode: Config.FILESYSTEM.DEFAULT_FILE_MODE, //
            mtime: nowISO, // Set initial mtime, will be confirmed by _updateNodeAndParentMtime
        };
      }

      // --- Update Timestamps and Save File System ---
