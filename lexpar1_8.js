// lexpar.js - OopisOS Lexer/Parser Logic v1.8 (Enhanced Escape Handling)
      ...
      while (this.position < this.input.length) {
        ...
        if (char === '\\') { 
          ...
          if (this.position < this.input.length) { 
            ...
            // Crucially, advance position past the character that was part of the escape sequence
            this.position++; 
          } else { 
            value += '\\'; 
            // Loop will naturally terminate as this.position is at input.length
          }
          ...
          value += char;
          this.position++; // Consume normal character
        }
      }
      // If loop finishes, string was unclosed
      throw new Error(`Lexer Error: Unclosed string literal starting at position ${startPos}. Expected closing ${quoteChar}.`);
    }
  }

  // --- Parser Class (ensure this matches your latest working version, e.g., from lexpar_v1_8_exec_fix or lexpar_v1_8_semicolon_fix2) ---
  ...
  class Parser {
    ...
    parse() {
      ...
      while(this._currentToken().type !== TokenType.EOF) {
          ...
          if (this._currentToken().type === TokenType.OPERATOR_SEMICOLON) {
            ...
            if (this._currentToken().type === TokenType.EOF) {
              if (allPipelines.length === 0) { // Handle cases like just ";" or " ; ; "
      ...
      if (allPipelines.length === 0 && this.tokens.length === 1 && this.tokens[0].type === TokenType.EOF) {
          // Input was truly empty (or just whitespace)
          return [];
      }
      if (allPipelines.length === 0) {
          // Input was like "; ;" or just " " resulting in no actual pipelines after processing
          // We should still return an array, perhaps with one empty pipeline if that's how CommandExecutor handles it.
          // Or an empty array if CommandExecutor handles that.
          // Let's ensure an empty array is returned for truly no operation.
          // If it was just ";", the loop might add an empty pipeline if not careful.
          // The logic above for handling trailing semicolon already adds an empty pipeline if allPipelines is empty.
      }
      return allPipelines;
    }
  }
