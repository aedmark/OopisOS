// commexec.js - OopisOS Command Executor v2.3 (Fully Refactored)
const CommandExecutor = (() => {
		...
		async function _handleUserSwitch(commandName, targetUser, providedPassword) {
			return new Promise(async(resolve) => {
				...
				if(initialLoginAttempt.requiresPasswordPrompt) {
					ModalInputManager.requestInput(Config.MESSAGES.PASSWORD_PROMPT, async(password) => {
		...
		function createCommandHandler(definition) {
			...
			return async(args, options) => {
				const {
					flags,
					remainingArgs
				} = Utils.parseFlags(args, definition.flagDefinitions || []);
				...
				if(definition.pathValidation) {
					for(const pv of definition.pathValidation) {
						...
						if(pathArg === undefined) {
							if(pv.optional) {
								continue;
								// Skip validation for this optional, missing path
							}
							return {
								success: false,
				...
				if(definition.permissionChecks) {
					for(const pc of definition.permissionChecks) {
						...
						if(!validatedPath.node) {
							return {
								success: false,
								error: `${definition.commandName || ''}: '${remainingArgs[pc.pathArgIndex]}': No such file or directory to check permissions.`
							};
						}
						for(const perm of pc.permissions) {
							if(!FileSystemManager.hasPermission(validatedPath.node, currentUser, perm)) {
								return {
									success: false,
									error: `${definition.commandName || ''}: '${remainingArgs[pc.pathArgIndex]}'${Config.MESSAGES.PERMISSION_DENIED_SUFFIX}`
								};
							}
						}
					}
		...
		const geminiCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				const _getApiKey = () => {
					return new Promise((resolve, reject) => {
						...
						if(options.isInteractive) {
							ModalInputManager.requestInput("Please enter your Gemini API key. It will be saved locally for future use.", (providedKey) => {
								if(!providedKey) {
								...
								OutputManager.appendToOutput("API Key saved to local storage.", {
						...
						function_declarations: [{
							...
							parameters: {
								...
								properties: {
									path: {
										type: "STRING",
										description: "The path of the directory or file to list. Defaults to the current directory."
									}
								}
							}
						}, {
							...
							parameters: {
								...
								properties: {
									path: {
										type: "STRING",
										description: "The path to the file whose content should be displayed."
									}
								}
							}
						}, {
							...
							parameters: {
								...
								properties: {
									...
									expression: {
										type: "STRING",
										description: "The search expression (e.g., '-name \"*.txt\"')."
									}
								}
							}
						}, {
							...
							parameters: {
								...
								properties: {
									...
									path: {
										type: "STRING",
										description: "The path of the file or directory to search in."
									}
								}
							}
						}, {
							...
							parameters: {
								...
								properties: {
									path: {
										type: "STRING",
										description: "The path of the directory to display as a tree. Defaults to the current directory."
									}
								}
							}
						}]
				...
				if(flags.new) {
					...
					if(options.isInteractive) {
						OutputManager.appendToOutput("Starting a new conversation with Gemini.", {
				...
				try {
					if(options.isInteractive && geminiConversationHistory.length <= 1) {
						OutputManager.appendToOutput("Gemini is thinking...", {
							typeClass: Config.CSS_CLASSES.CONSOLE_LOG_MSG
						});
					}
					while(true) {
						...
						if(!candidate || !candidate.content || !candidate.content.parts) {
							...
							return {
								success: false,
								error: "gemini: Received an invalid or empty response from the API."
							};
						}
						let textResponse = "";
						...
						if(functionCall) {
							...
							if(options.isInteractive) {
								...
								OutputManager.appendToOutput(`Gemini is exploring with: ${funcName}(${funcArgs})`, {
									typeClass: Config.CSS_CLASSES.CONSOLE_LOG_MSG
								});
							}
							...
							geminiConversationHistory.push({
								...
								parts: [{
									functionResponse: {
										name: commandName,
										response: {
											content: execResult.success ? (execResult.output || "(No output from command)") : `Error: ${execResult.error || "Command failed"}`
										}
									}
								}]
							});
					...
					return {
						success: false,
						error: `gemini: An error occurred. ${e.message}`
					};
				}
			}
		};
    ...
    return {
		...
		const mvCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				const sourceValidation = FileSystemManager.validatePath("mv (source)", sourcePathArg, {
					disallowRoot: true
				});
				if(sourceValidation.error) return {
					success: false,
				...
				const destValidation = FileSystemManager.validatePath("mv (destination)", destPathArg, {
					allowMissing: true
				});
			...
			coreLogic: async(context) => {
				...
				try {
					const firstPassword = await new Promise((resolve, reject) => {
						ModalInputManager.requestInput(Config.MESSAGES.PASSWORD_PROMPT, (pwd) => resolve(pwd), () => reject(new Error(Config.MESSAGES.OPERATION_CANCELLED)), true // <<< isObscured: true
						);
					});
					if(firstPassword.trim() === '') {
						OutputManager.appendToOutput("Registering user with no password.", {
							typeClass: Config.CSS_CLASSES.CONSOLE_LOG_MSG
						});
					} else {
						const confirmedPassword = await new Promise((resolve, reject) => {
							ModalInputManager.requestInput(Config.MESSAGES.PASSWORD_CONFIRM_PROMPT, (pwd) => resolve(pwd), () => reject(new Error(Config.MESSAGES.OPERATION_CANCELLED)), true // <<< isObscured: true
							);
						});
					...
					if(registerResult.success) {
						OutputManager.appendToOutput(registerResult.message, {
							typeClass: Config.CSS_CLASSES.SUCCESS_MSG
						});
						return {
							success: true,
			...
			coreLogic: async(context) => {
				...
				return {...result,
					output: result.message,
						messageType: result.success ? Config.CSS_CLASSES.SUCCESS_MSG : Config.CSS_CLASSES.CONSOLE_LOG_MSG,
				};
			}
		};
		...
		const clearfsCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				const confirmed = await new Promise((resolve) => ModalManager.request({
					context: 'terminal',
					messageLines: [`WARNING: This will permanently erase ALL files and directories in your home directory (${userHomePath}).`, "This action cannot be undone.", "Are you sure you want to clear your home directory?", ],
					onConfirm: () => resolve(true),
					onCancel: () => resolve(false),
				}), );
				if(!confirmed) {
				...
				OutputManager.appendToOutput(successMessage, {
					typeClass: Config.CSS_CLASSES.SUCCESS_MSG
				});
				return {
					success: true,
		...
		const resetCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				if(confirmed) {
					...
					return {
						success: true,
						output: "OopisOS reset to initial state. Please refresh the page if UI issues persist.",
						messageType: Config.CSS_CLASSES.SUCCESS_MSG
					};
				} else {
					return {
    ...
    // Actively poll the signal instead of passively listening
    while (Date.now() - startTime < ms) {
        if (signal?.aborted) {
            // The signal has been flipped, terminate immediately.
            return {
                success: false,
                error: `delay: Operation cancelled. (Reason: ${signal.reason})`
            };
        }
        // Yield to the event loop for 50ms to prevent blocking the browser
        await new Promise(resolve => setTimeout(resolve, 50));
    }

    // This code is only reachable if the loop completes without being aborted.
    if (options.isInteractive && !CommandExecutor.isScriptRunning()) {
    ...
    return { success: true, output: "" };
		...
		const removeuserCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				if(FileSystemManager.getNodeByPath(userHomePath)) {
					const rmResult = await FileSystemManager.deleteNodeRecursive(userHomePath, {
						force: true,
		...
		const lsCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				function sortItems(items, currentFlags) {
					...
					if(currentFlags.noSort) {} else if(currentFlags.sortByTime) {
		...
		const treeCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				const maxDepth = flags.level ? Utils.parseNumericArg(flags.level, {
					min: 0
				}) : {
					value: Infinity
				};
				if(flags.level && (maxDepth.error || maxDepth.value === null)) return {
					success: false,
				...
				if(!FileSystemManager.hasPermission(pathValidation.node, currentUser, "read")) return {
					success: false,
				...
				function buildTreeRecursive(currentDirPath, currentDepth, indentPrefix) {
					...
					childrenNames.forEach((childName, index) => {
						...
						if(childNode.type === Config.FILESYSTEM.DEFAULT_DIRECTORY_TYPE) {
							...
							if(currentDepth < maxDepth.value) buildTreeRecursive(FileSystemManager.getAbsolutePath(childName, currentDirPath), currentDepth + 1, indentPrefix + (index === childrenNames.length - 1 ? "    " : "│   "));
		...
		const touchCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				if(timestampResult.error) return {
					success: false,
				...
				let changesMade = false;
				for(const pathArg of args) {
					const pathValidation = FileSystemManager.validatePath("touch", pathArg, {
						allowMissing: true,
						disallowRoot: true
					});
			...
			coreLogic: async(context) => {
				...
				async function _executeCopyInternal(sourceNode, sourcePathForMsg, targetContainerAbsPath, targetEntryName, currentCommandFlags) {
					...
					if(!targetContainerNode || targetContainerNode.type !== Config.FILESYSTEM.DEFAULT_DIRECTORY_TYPE) {
						return {
							success: false,
							messages: [`cp: target '${targetContainerAbsPath}' is not a directory.`],
							changesMade: false
						};
					}
					if(!FileSystemManager.hasPermission(targetContainerNode, currentUser, "write")) {
						return {
							success: false,
							messages: [`cp: cannot create item in '${targetContainerAbsPath}'${Config.MESSAGES.PERMISSION_DENIED_SUFFIX}`],
							changesMade: false
						};
					}
					...
					if(existingNodeAtDest) {
						if(sourceNode.type !== existingNodeAtDest.type) {
							return {
								success: false,
								messages: [`cp: cannot overwrite ${existingNodeAtDest.type} '${fullFinalDestPath}' with ${sourceNode.type} '${sourcePathForMsg}'`],
								changesMade: false
							};
						}
						if(currentCommandFlags.isInteractiveEffective) {
							...
							if(!confirmed) return {
								success: true,
								messages: [`cp: not overwriting '${fullFinalDestPath}' (skipped)`],
								changesMade: false
							};
						} else if(!currentCommandFlags.force) {
							return {
								success: false,
								messages: [`cp: '${fullFinalDestPath}' already exists. Use -f to overwrite or -i to prompt.`],
								changesMade: false
							};
						}
					}
					if(sourceNode.type === Config.FILESYSTEM.DEFAULT_FILE_TYPE) {
						const newFileNode = {
							...
							owner: currentCommandFlags.preserve ? sourceNode.owner : currentUser,
							mode: currentCommandFlags.preserve ? sourceNode.mode : Config.FILESYSTEM.DEFAULT_FILE_MODE,
						...
						if(!currentCommandFlags.recursive) {
							return {
								success: true,
								messages: [`cp: omitting directory '${sourcePathForMsg}' (use -r or -R)`],
								changesMade: false
							};
						}
						let destDirNode = existingNodeAtDest || {
							type: Config.FILESYSTEM.DEFAULT_DIRECTORY_TYPE,
							children: {},
							owner: currentCommandFlags.preserve ? sourceNode.owner : currentUser,
							mode: currentCommandFlags.preserve ? sourceNode.mode : Config.FILESYSTEM.DEFAULT_DIR_MODE,
							mtime: currentCommandFlags.preserve ? sourceNode.mtime : nowISO
						};
				...
				const destValidation = FileSystemManager.validatePath("cp (dest)", rawDestPathArg, {
					allowMissing: true
				});
				if(destValidation.error && !(destValidation.optionsUsed.allowMissing && !destValidation.node)) {
					return {
				...
				return {
					...
					error: overallSuccess ? null : finalMessages || "An unknown error occurred."
				};
			}
		};
		const historyCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				return {
					success: true,
					output: history.map((cmd, i) => `  ${String(i + 1).padStart(3)}  ${cmd}`).join("\n")
				};
			}
		};
		const grepCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				if(args.length === 0 && options.stdinContent === null) return {
					success: false,
				...
				async function searchRecursively(currentPath, displayPathArg) {
					...
					if(!FileSystemManager.hasPermission(node, currentUser, "read")) {
						OutputManager.appendToOutput(`grep: ${displayPathArg}${Config.MESSAGES.PERMISSION_DENIED_SUFFIX}`, {
							typeClass: Config.CSS_CLASSES.ERROR_MSG
						});
						overallSuccess = false;
						return;
					...
					if(node.type === Config.FILESYSTEM.DEFAULT_FILE_TYPE) {
						...
						if(!flags.recursive) {
							OutputManager.appendToOutput(`grep: ${displayPathArg}: Is a directory`, {
								typeClass: Config.CSS_CLASSES.ERROR_MSG
							});
							overallSuccess = false;
							return;
				...
				if(filePathsArgs.length > 0) {
					for(const pathArg of filePathsArgs) {
						if(flags.recursive) {
							...
							const pathValidation = FileSystemManager.validatePath("grep", pathArg, {
								expectedType: Config.FILESYSTEM.DEFAULT_FILE_TYPE
							});
							if(pathValidation.error) {
								OutputManager.appendToOutput(pathValidation.error, {
							...
							if(!FileSystemManager.hasPermission(pathValidation.node, currentUser, "read")) {
								OutputManager.appendToOutput(`grep: ${pathArg}${Config.MESSAGES.PERMISSION_DENIED_SUFFIX}`, {
									typeClass: Config.CSS_CLASSES.ERROR_MSG
								});
								overallSuccess = false;
								continue;
		...
		const uploadCommandDefinition = {
			...
			coreLogic: async(context) => {
				...
				if(args.length === 1) {
					const destPathValidation = FileSystemManager.validatePath("upload (destination)", args[0], {
						expectedType: Config.FILESYSTEM.DEFAULT_DIRECTORY_TYPE
					});
					if(destPathValidation.error) return {
						success: false,
				...
				if(!targetDirNode || !FileSystemManager.hasPermission(targetDirNode, currentUser, "write")) return {
					success: false,
				...
				try {
					const filesToUpload = await new Promise((resolve, reject) => {
						...
						input.onchange = (e) => {
							dialogClosed = true;
							window.removeEventListener("focus", onFocus);
							if(e.target.files ?.length > 0) resolve(e.target.files);
		...
		const restoreCommandDefinition = {
			...
			coreLogic: async(context) => {
				if(!context.options.isInteractive) return {
					success: false,
				...
				try {
					const file = await new Promise((res, rej) => {
						...
						const onFocus = () => {
							setTimeout(() => {
								...
								if(!dialogClosed) {
									dialogClosed = true;
									rej(new Error(Config.MESSAGES.RESTORE_CANCELLED_NO_FILE));
								}
							}, 300);
						};
						window.addEventListener("focus", onFocus);
					...
					const messageLines = [`WARNING: This will completely overwrite the current OopisOS state.`, `All users, files, and sessions will be replaced with data from '${file.name}'.`, "This action cannot be undone. Are you sure you want to restore?"];
					const confirmed = await new Promise((conf) => ModalManager.request({
						context: 'terminal',
						messageLines,
						onConfirm: () => conf(true),
						onCancel: () => conf(false),
					...
					OutputManager.appendToOutput("System state restored successfully. Rebooting OopisOS to apply changes...", {
						typeClass: Config.CSS_CLASSES.SUCCESS_MSG
					});
					setTimeout(() => {
						window.location.reload(true);
					...
					// Output won't be seen due to reload, but good practice.
				} catch(e) {
					return {
						success: false,
			...
			coreLogic: async(context) => {
				...
				const predicates = {
					"-name": (node, path, pattern) => {
						...
						if(!regex) {
							OutputManager.appendToOutput(`find: invalid pattern for -name: ${pattern}`, {
								typeClass: Config.CSS_CLASSES.ERROR_MSG
							});
							overallSuccess = false;
							return false;
						}
						return regex.test(path.substring(path.lastIndexOf(Config.FILESYSTEM.PATH_SEPARATOR) + 1));
					},
					"-type": (node, path, typeChar) => {
						...
						OutputManager.appendToOutput(`find: unknown type '${typeChar}' for -type`, {
							typeClass: Config.CSS_CLASSES.ERROR_MSG
						});
						overallSuccess = false;
						return false;
					...
					"-perm": (node, path, modeStr) => {
						if(!/^[0-7]{2}$/.test(modeStr)) {
							OutputManager.appendToOutput(`find: invalid mode '${modeStr}' for -perm`, {
								typeClass: Config.CSS_CLASSES.ERROR_MSG
							});
							overallSuccess = false;
							return false;
				...
				const actions = {
					...
					"-exec": async(node, path, commandParts) => {
						...
						if(!result.success) {
							OutputManager.appendToOutput(`find: -exec: command '${cmdStr}' failed: ${result.error}`, {
					...
					"-delete": async(node, path) => {
						...
						if(!result.success) {
							OutputManager.appendToOutput(`find: -delete: ${result.messages.join(';') || `
				...
				while(i < expressionArgs.length) {
					...
					if(token === "-or" || token === "-o") {
						if(currentTermGroup.length > 0) parsedExpression.push({
							type: "AND_GROUP",
					...
					if(predicates[token]) {
						...
						if(token === "-exec") {
							...
							if(i >= expressionArgs.length || expressionArgs[i] !== ";") return {
								success: false,
				...
				if(!hasExplicitAction) {
					if(parsedExpression.length === 0 || parsedExpression[parsedExpression.length - 1].type === "OR") parsedExpression.push({
						type: "AND_GROUP",
						terms: []
					});
				...
				async function evaluateExpressionForNode(node, path) {
					...
					for(const groupOrOperator of parsedExpression) {
						if(groupOrOperator.type === "OR") {
							...
							if(isFirstAndGroup) orGroupValue = andSubResult;
							else orGroupValue = orGroupValue && andSubResult;
						}
					}
					return orGroupValue;
				}
				async function recurseFind(currentResolvedPath, isDepthFirst) {
					...
					if(!node) {
						OutputManager.appendToOutput(`find: ‘${currentResolvedPath}’: No such file or directory`, {
							typeClass: Config.CSS_CLASSES.ERROR_MSG
						});
						filesProcessedSuccessfully = false;
						return;
					...
					if(!FileSystemManager.hasPermission(node, currentUser, "read")) {
						OutputManager.appendToOutput(`find: ‘${currentResolvedPath}’: Permission denied`, {
							typeClass: Config.CSS_CLASSES.ERROR_MSG
						});
						filesProcessedSuccessfully = false;
						return;
				...
				if(startPathValidation.error) return {
					success: false,
		...
		async function _executeCommandHandler(segment, execCtxOpts, stdinContent = null, signal) {
			...
			if(cmdData ?.handler) {
				try {
					...
					return await cmdData.handler(segment.args, {...execCtxOpts,
						stdinContent,
						signal
					});
				} catch(e) {
		...
		async function _executePipeline(pipeline, isInteractive, signal) {
			...
			for(let i = 0; i < pipeline.segments.length; i++) {
				...
				// Pass the signal down to the handler executor
				lastResult = await _executeCommandHandler(segment, {
					isInteractive
				}, currentStdin, signal);
				if(!lastResult) {
			...
			if(pipeline.redirection && lastResult.success) {
				...
				const redirVal = FileSystemManager.validatePath("redirection", redirFile, {
					allowMissing: true,
					disallowRoot: true,
					defaultToCurrentIfEmpty: false
				});
				if(redirVal.error && !(redirVal.optionsUsed.allowMissing && !redirVal.node)) {
					if(!pipeline.isBackground) await OutputManager.appendToOutput(redirVal.error, {
						typeClass: Config.CSS_CLASSES.ERROR_MSG
					});
					return {
						success: false,
				...
				if(pDirRes.error) {
					if(!pipeline.isBackground) await OutputManager.appendToOutput(`Redir err: ${pDirRes.error}`, {
						typeClass: Config.CSS_CLASSES.ERROR_MSG
					});
					return {
						success: false,
				...
				if(!finalParentNodeForFile) {
					if(!pipeline.isBackground) await OutputManager.appendToOutput(`Redir err: critical internal error, parent dir '${finalParentDirPath}' for file write not found.`, {
						typeClass: Config.CSS_CLASSES.ERROR_MSG
					});
					return {
						success: false,
				...
				if(targetNode && targetNode.type === Config.FILESYSTEM.DEFAULT_DIRECTORY_TYPE) {
					if(!pipeline.isBackground) await OutputManager.appendToOutput(`Redir err: '${redirFile}' is dir.`, {
						typeClass: Config.CSS_CLASSES.ERROR_MSG
					});
					return {
						success: false,
				...
				if(targetNode && !FileSystemManager.hasPermission(targetNode, user, "write")) {
					if(!pipeline.isBackground) await OutputManager.appendToOutput(`Redir err: no write to '${redirFile}'${Config.MESSAGES.PERMISSION_DENIED_SUFFIX}`, {
						typeClass: Config.CSS_CLASSES.ERROR_MSG
					});
					return {
						success: false,
				...
				if(!targetNode && !FileSystemManager.hasPermission(finalParentNodeForFile, user, "write")) {
					if(!pipeline.isBackground) await OutputManager.appendToOutput(`Redir err: no create in '${finalParentDirPath}'${Config.MESSAGES.PERMISSION_DENIED_SUFFIX}`, {
						typeClass: Config.CSS_CLASSES.ERROR_MSG
					});
					return {
						success: false,
				...
				if(targetNode) {
					...
					if(newFileNode) {
						...
						if(!pipeline.isBackground) await OutputManager.appendToOutput(`Redirection error: internal failure creating new file node for '${redirFile}'.`, {
							typeClass: Config.CSS_CLASSES.ERROR_MSG
						});
						return {
							success: false,
							error: "internal redirection error"
						};
					}
				}
				FileSystemManager._updateNodeAndParentMtime(absRedirPath, nowISO);
				if(!(await FileSystemManager.save(user))) {
					if(!pipeline.isBackground) await OutputManager.appendToOutput(`Failed to save redir to '${redirFile}'.`, {
						typeClass: Config.CSS_CLASSES.ERROR_MSG
					});
					return {
						success: false,
			...
			if(!pipeline.redirection && lastResult.success && lastResult.output !== null && lastResult.output !== undefined) {
				if(!pipeline.isBackground) {
					...
					await OutputManager.appendToOutput(`${Config.MESSAGES.BACKGROUND_PROCESS_OUTPUT_SUPPRESSED} (Job ${pipeline.jobId})`, {
						typeClass: Config.CSS_CLASSES.CONSOLE_LOG_MSG,
						isBackground: true
					});
				}
		...
		async function processSingleCommand(rawCommandText, isInteractive = true) {
			...
			if(scriptExecutionInProgress && isInteractive && !ModalManager.isAwaiting()) {
				OutputManager.appendToOutput("Script execution in progress. Input suspended.", {
					typeClass: Config.CSS_CLASSES.WARNING_MSG
				});
				return {
					success: false,
			...
			try {
				...
				parsedPipelines = new Parser(new Lexer(commandToProcess).tokenize()).parse();
			...
			for(const pipeline of parsedPipelines) {
				...
				if(pipeline.isBackground) {
					...
					await OutputManager.appendToOutput(`${Config.MESSAGES.BACKGROUND_PROCESS_STARTED_PREFIX}${pipeline.jobId}${Config.MESSAGES.BACKGROUND_PROCESS_STARTED_SUFFIX}`, {
						typeClass: Config.CSS_CLASSES.CONSOLE_LOG_MSG
					});
					overallResult = {
						success: true,
					...
					setTimeout(async() => {
						try {
							const bgResult = await _executePipeline(pipeline, false, abortController.signal);
							...
							if(!bgResult.success) console.log(`Background job ${jobId} error details: ${bgResult.error || "No specific error message."}`);
						} finally {
