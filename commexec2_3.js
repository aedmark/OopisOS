...
const CommandExecutor = (() => {
		...
		function createCommandHandler(definition) {
			...
			return async (args, options) => {
				...
				const context = {
					...
					args: remainingArgs,
					// Pass flag-less args to core logic
					options,
					flags,
					// Pass parsed flags
					currentUser,
					...
					while (true) {
						...
						if (functionCall) {
							...
							const commandArgs = Object.values(functionCall.args || {})
								.map(arg => typeof arg === 'string' ? `"${arg}"` : arg).join(" ");
    ...
    if (args.length === 0) {
        ...
        return { success: true, output: outputLines.sort().join("\n") };
    }
    ...
    if (eqIndex !== -1) {
        ...
        if (!name) {
            return { success: false, error: "alias: invalid format. Missing name." };
        }
        ...
        return { success: false, error: "alias: failed to set alias." };
    }
    
    // Case 3 (NEW): `alias name1 name2 ...` (display specific aliases)
    else {
        const outputLines = [];
        let allFound = true;
        for (const name of args) {
            ...
            if (value) {
                ...
                OutputManager.appendToOutput(`alias: ${name}: not found`, { typeClass: Config.CSS_CLASSES.ERROR_MSG });
                allFound = false;
            }
        }
        return { success: allFound, output: outputLines.join("\n") };
    }
}
	};
			...
			coreLogic: async (context) => {
				...
				const actions = {
					...
					"-delete": async (node, path) => {
						...
						if (!result.success) {
							OutputManager.appendToOutput(`find: -delete: ${result.messages.join(';') || `failed to delete '${path}'`}`, {
								typeClass: Config.CSS_CLASSES.WARNING_MSG
							});
							filesProcessedSuccessfully = false;
							return false;
		...
		for (const pipeline of parsedPipelines) {
			...
			if (pipeline.isBackground) {
				...
				setTimeout(async () => {
					...
					const statusMsg = `[Job ${pipeline.jobId} ${bgResult.success ? "finished" : "finished with error"}${bgResult.success ? "" : `: ${
							bgResult.error || "Unknown error"
						}
					`}]`;
